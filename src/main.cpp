...
#if defined(NDEBUG)
# error "Lifefuturecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x463d85b7eb09d03c6d0b61aaef98a96c9eb4510de135d0749fbf5f7b9f9f8f87");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Lifefuturecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Lifefuturecoin Signed Message:\n";
...
{
    // Lifefuturecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1); // Bitfilcoin: 840k blocks in ~4 years
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Lifefuturecoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // Lifefuturecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Lifefuturecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Lifefuturecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd52447f925b3d32e0381b3180cb664048ded37be5831d9f67dbd1604d87d1aa6");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04c2475e4d84b74c87c98cba801c2232822f4634cdf51329fcaf24811daa54024b42a6f474b35418608789924a2c8b56ecdd07a380b75b7dd52342a6d5de00aeb6") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1534256942;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084683937;

        if (fTestNet)
        {
            block.nTime    = 1534256928;
            block.nNonce   = 386155380;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x1facddac71868b0aa985950c5d7f18cfdf73adb9d1c8ac64fe2e5d7c0b9fd354"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xbe, 0xd3 }; // Lifefuturecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LifefuturecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LifefuturecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LifefuturecoinMiner(CWallet *pwallet)
{
    printf("LifefuturecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lifefuturecoin-miner");
    ...
    try { loop {
        ...
        printf("Running LifefuturecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LifefuturecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LifefuturecoinMiner, pwallet));
}
